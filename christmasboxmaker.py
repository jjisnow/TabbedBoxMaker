...
def drawS(XYstring):  # Draw lines from a list
    ...
    drw = {'style'                         : simplestyle.formatStyle(style),
           inkex.addNS('label', 'inkscape'): name, 'd': XYstring}
...
def side((rx, ry), (sox, soy), (eox, eoy), tabVec, length, (dirx, diry), isTab, isDivider,
         numDividers, divSpacing, divOffset):
    #       root startOffset endOffset tabVec length  direction  isTab isDivider
    # numDividers divSpacing dividerOffset
  ...
  s='M '+str(Vx)+','+str(Vy)+' '
    ...
    for n in range(1, int(divs)):
        if ((n % 2) ^ (
        not isTab)) and numDividers > 0 and not isDivider:  # draw holes for divider
          # joints in side walls
            ...
            for m in range(1, int(numDividers) + 1):
                ...
                drawS(h)
        if n % 2:
            if n == 1 and numDividers > 0 and isDivider:  # draw slots for dividers to
    ...
    # finish the line off
    s += 'L ' + str(rx + eox * thickness + dirx * length) + ',' + str(
        ry + eoy * thickness + diry * length) + ' '
    if isTab and numDividers > 0 and not isDivider:  # draw last for divider joints in
      # side walls
        for m in range(1, int(numDividers) + 1):
            Dx = Vx
            Dy = Vy + dirx * divSpacing * m
...
class BoxMaker(inkex.Effect):
    ...
    def effect(self):
        global parent, nomTab, equalTabs, thickness, correction, divx, divy, hairline, \
          linethickness, keydivwalls, keydivfloor
    ...
    bevel = self.unittouu( str(self.options.bevel) + unit )thickness = self.unittouu( str(self.options.thickness)  + unit )
    ...
    if boxtype==2: # One side open (X,Y)
      if layout==1: # Diagramatic Layout
        pieces=[[(2,0,0,1),(3,0,1,1),X,Z,0b1010,0b1101,2],[(1,0,0,0),(2,0,0,1),Z,Y,0b1111,0b1110,3],
                ...
                [(0,0,0,0.50),(10,0,1,1),bevel+thickness,Z-bevel*2,0b0000,0b0000,2]]
      ...
      if   layout==1: # Diagramatic Layout
        ...
        for idx, piece in enumerate(pieces):  # generate and draw each piece of the box
            ...
            drawS(side((x, y), (d, a), (-b, a), atabs * (-thickness if a else thickness),
                       dx, (1, 0), a, 0, (keydivfloor | wall) * (
                       keydivwalls | floor) * divx * yholes * atabs, yspacing,
                       divOffset))  # side a
            drawS(side((x + dx, y), (-b, a), (-b, -c),
                       btabs * (thickness if b else -thickness), dy, (0, 1), b, 0,
                       (keydivfloor | wall) * (
                       keydivwalls | floor) * divy * xholes * btabs, xspacing,
                       divOffset))  # side b
            if atabs:
                drawS(side((x + dx, y + dy), (-b, -c), (d, -c),
                           ctabs * (thickness if c else -thickness), dx, (-1, 0), c, 0, 0,
                           0, divOffset))  # side c
            else:
                drawS(side((x + dx, y + dy), (-b, -c), (d, -c),
                           ctabs * (thickness if c else -thickness), dx, (-1, 0), c, 0,
                           (keydivfloor | wall) * (
                           keydivwalls | floor) * divx * yholes * ctabs, yspacing,
                           divOffset))  # side c
            if btabs:
                drawS(side((x, y + dy), (d, -c), (d, a),
                           dtabs * (-thickness if d else thickness), dy, (0, -1), d, 0, 0,
                           0, divOffset))  # side d
            else:
                drawS(side((x, y + dy), (d, -c), (d, a),
                           dtabs * (-thickness if d else thickness), dy, (0, -1), d, 0,
                           (keydivfloor | wall) * (
                           keydivwalls | floor) * divy * xholes * dtabs, xspacing,
                           divOffset))  # side d
            ...
            if idx == 0:
                ...
                for n in range(0, divx):  # generate X dividers
                    ...
                    drawS(side((x, y), (d, a), (-b, a),
                               keydivfloor * atabs * (-thickness if a else thickness), dx,
                               (1, 0), a, 1, 0, 0, divOffset))  # side a
                    drawS(side((x + dx, y), (-b, a), (-b, -c), keydivwalls * btabs * (
                    thickness if keydivwalls * b else -thickness), dy, (0, 1), b, 1,
                               divy * xholes, xspacing, divOffset))  # side b
                    drawS(side((x + dx, y + dy), (-b, -c), (d, -c),
                               keydivfloor * ctabs * (thickness if c else -thickness), dx,
                               (-1, 0), c, 1, 0, 0, divOffset))  # side c
                    drawS(side((x, y + dy), (d, -c), (d, a),
                               keydivwalls * dtabs * (-thickness if d else thickness), dy,
                               (0, -1), d, 1, 0, 0, divOffset))  # side d
            elif idx == 1:
                ...
                for n in range(0, divy):  # generate Y dividers
                    ...
                    drawS(side((x, y), (d, a), (-b, a),
                               keydivwalls * atabs * (-thickness if a else thickness), dx,
                               (1, 0), a, 1, divx * yholes, yspacing,
                               thickness))  # side a
                    drawS(side((x + dx, y), (-b, a), (-b, -c),
                               keydivfloor * btabs * (thickness if b else -thickness), dy,
                               (0, 1), b, 1, 0, 0, thickness))  # side b
                    drawS(side((x + dx, y + dy), (-b, -c), (d, -c),
                               keydivwalls * ctabs * (thickness if c else -thickness), dx,
                               (-1, 0), c, 1, 0, 0, thickness))  # side c
                    drawS(side((x, y + dy), (d, -c), (d, a),
                               keydivfloor * dtabs * (-thickness if d else thickness), dy,
                               (0, -1), d, 1, 0, 0, thickness))  # side d
